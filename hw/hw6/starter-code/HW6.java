...
public class HW6 {
  ...
  public static Dataset<Row> Q1 (SparkSession spark, String dataFile) {
    ...
    // your code here
    df.createOrReplaceTempView("flights");
    Dataset<Row> r = spark.sql("SELECT DISTINCT F.destcityname FROM flights AS F WHERE F.origincityname = 'Seattle, WA'"); 
    r.show();
    return r;
  }
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    // your code here
    JavaRDD<Row> f = d.filter(t -> t.getString(ORIGIN_CITY_NAME).equals("Seattle, WA"));
    JavaRDD<Object> m = f.map(t -> t.getString(DEST_CITY_NAME));
    JavaRDD<Row> r = m.map(RowFactory::create).distinct();
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    JavaPairRDD<Tuple2<String, Integer>, Integer> r = nc.mapToPair(t ->
      new Tuple2<Tuple2<String, Integer>, Integer>(
        new Tuple2<String, Integer>(t.getAs(ORIGIN_CITY_NAME), t.getAs(MONTH)), 1))
    .reduceByKey((a, b) -> a + b);
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    // your code here
    JavaPairRDD<String, String> p1 = d.mapToPair(t -> 
      new Tuple2<String, String>(t.getAs(ORIGIN_CITY_NAME), t.getAs(DEST_CITY_NAME)))
    .distinct();

    JavaPairRDD<String, Integer> p2 = p1.mapToPair(t -> 
      new Tuple2<String,Integer>(t._1, 1))
    .reduceByKey((a, b) -> a + b);

    Tuple2<String,Integer> max = p2.max(new IntComparator());

    JavaRDD<Row> f = d.filter(t -> t.getAs(ORIGIN_CITY_NAME).equals(max._1));

    JavaPairRDD<String, Integer> r = f.mapToPair(t -> 
      new Tuple2<String, Integer>(t.getAs(ORIGIN_CITY_NAME), 1)).reduceByKey((a, b) -> a + b).first();

    return r;
  }

  private static class IntComparator implements Comparator<Tuple2<String, Integer>>, Serializable {
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    ...
    // your code here
    JavaRDD<Row> f = d.filter(t -> t.getAs(DEP_DELAY) != null);

    JavaPairRDD<String, Tuple2<Integer, Integer>> p = f.mapToPair(t -> 
      new Tuple2<String, Tuple2<Integer, Integer>>(t.getAs(ORIGIN_CITY_NAME), new Tuple2<Integer, Integer>(t.getAs(DEP_DELAY), 1)))
    .reduceByKey((a,b) -> new Tuple2<Integer, Integer>(a._1 + b._1, a._2 + b._2));

    JavaPairRDD<String, Double> r = p.mapToPair(t -> new Tuple2<String, Double>(t._1, t._2._1 * 1.0 / t._2._2));

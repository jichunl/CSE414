...
public class HW6 {
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    // your code here
	JavaRDD result = d.filter(t -> 
		{return t.getString(ORIGIN_CITY_NAME).compareTo("Seattle, WA") == 0;}).map(t -> 
		{return RowFactory.create(t.getString(DEST_CITY_NAME));}).distinct();
    return result;
  }
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    // your code here
	JavaRDD<Row> nc = d.filter(t -> t.getInt(CANCELLED) == 0);
	JavaPairRDD<Tuple2<String, Integer>, Integer> pair = 
	nc.mapToPair(t -> new Tuple2<Tuple2<String, Integer>, Integer>(new Tuple2<String, Integer>(t.getAs(ORIGIN_CITY_NAME), t.getAs(MONTH)), 1));
	JavaPairRDD<Tuple2<String, Integer>, Integer> counts = pair.reduceByKey((x,y) -> x + y); 
	return counts;
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    // your code here
	JavaPairRDD<String, String> pair = d.mapToPair(t -> new Tuple2<String, String>
									   (t.getAs(ORIGIN_CITY_NAME), t.getAs(DEST_CITY_NAME))).distinct(); 
	JavaPairRDD<String, Integer> p = pair.mapToPair(t -> new Tuple2<String, Integer>(t._1, 1)).reduceByKey((x,y) -> x + y);
    	Tuple2<String, Integer> result = p.max(new IntegerComparator());
	String city = result._1;
	JavaRDD<Row> e = d.filter(t -> t.getAs(ORIGIN_CITY_NAME).equals(city));
	JavaPairRDD<String, Integer> pair2 = e.mapToPair(t -> new Tuple2<String, Integer>(t.getAs(ORIGIN_CITY_NAME),1)).reduceByKey((x,y) -> x +y);
    	return pair2.first();
  }
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    ...
    // your code here
	JavaRDD<Row> e = d.filter(t -> t.getAs(DEP_DELAY) != null);
	JavaPairRDD<String, Tuple2<Integer, Integer>> pair = e.mapToPair(t -> new Tuple2<String, Tuple2<Integer, Integer>>(t.getAs(ORIGIN_CITY_NAME), new Tuple2<Integer, Integer>(t.getAs(DEP_DELAY), 1))).reduceByKey((x,y) -> new Tuple2<Integer, Integer>(x._1 + y._1, x._2 + y._2));
	JavaPairRDD<String, Double> result = pair.mapToPair(t -> new Tuple2<String, Double>(t._1, 1.0 * t._2._1 / t._2._2));
	return result;
  }

  private static class IntegerComparator implements Comparator<Tuple2<String, Integer>>, Serializable {
